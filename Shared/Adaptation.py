...
class MetaLearner:
    ...
    def train(self, X_train, y_train, X_test, y_test, get_data_Meta, N_way, K_shot):
        ...
        for meta_iter in range(self.meta_iters):
            ...
            train_data, test_data = get_data_Meta(X_train, y_train, X_test, y_test, N_way=N_way, K_shot=K_shot)
            ...
            result = self.model.fit(train_data[0], train_data[1], epochs=5, validation_data=(X_test, y_test), verbose=0)
...
def get_data_Meta(X_train, y_train, X_test, y_test, N_way, K_shot):
